...
beforeAfterFnt before after fn fnt = ParensE $ appBefore fnt where
...
marshalImport :: String -> Name -> Q [Dec]
marshalImport cfnstr fntn = do
  TyConI (TySynD fntn [] fnt) <- reify fntn
  let c:cs = nameBase fntn
      fnstr = toLower c : cs
      fn = mkName fnstr
      cfn = mkName $ "c_" ++ fnstr
      fndef = beforeAfterFnt 'marshal 'unmarshal' cfn fnt
  return
    [ ForeignD . ImportF CCall Safe cfnstr cfn $ AppT (ConT ''Marshalled) fnt
    , SigD fn fnt
